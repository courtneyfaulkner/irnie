...
public class Statement implements IQuery {
	 ...
	 * 
	 */
	public Statement(IConnection conn) throws OdaException {
	...
	public void prepare(String qry) throws OdaException {
		...
		List<String> arColsType = new ArrayList<String>(); // holds the column
															// types
		List<String> arColsName = new ArrayList<String>(); // holds the column
															// name
		List<String> arColEntity = new ArrayList<String>(); // holds the column
															// class

		try {
			// reading all persistence.xml class Nodes
			String persist = JPAUtil.getApplication().concat(
					CommonConstant.PERSISTENCE_XML);
			// List<Node> classNodes =
			// JPAUtil.findNodeByName(CommonConstant.PERSISTENCE_XML, "class");
			...
			// System.out.println("Se extrejeron: " + columnList.size()+
			// " columnas");
			...
			for (List<String> column : columnList) {
				...
				String entityNameLarge = JPAUtil.findEntityOnPersistenceXML(
						entityName, classNodes);
				String columnTypeLarge = JPAUtil.getFieldType(entityNameLarge,
						columnName);
		...
		String[] arLabels = (String[]) arColsName.toArray(new String[arColsName
				.size()]);
		...
		this.resultSetMetaData = new ResultSetMetaData(arLabels,
				...
				arLabels, (String[]) arColEntity.toArray(new String[arColEntity
						.size()]));
	...
	public static List<List<String>> extractColumns(final String qry)
		...
		if (selectPosition >= 0) {
			...
			/*
			 * for(List<String> l : columnList){ for(String s: l)
			 * System.out.print(s+" "); System.out.println(); }
			 */
			return columnList;
		} else {
			throw new OdaException(
					Messages.getString("Statement.QUERY_STRUCTURE_ERROR"));
			// return null;
		}
	}
	...
	public List<List<String>> getReturnEntities(String query)
		...
		if (fromPosition >= 6) {
			...
			/*
			 * for(List<String> l : entityList){ for(String s: l)
			 * System.out.print( s+" " ); System.out.println( ); }
			 */
			return entityList;
		} else {
	...
	public IResultSet executeQuery() throws OdaException {
		...
		// create a new ResultSet Object passing in the row set and the meta
		// data and the
	 ...
	 * 
	 * @see
	 * org.eclipse.birt.data.oda.IQuery#setSortSpec(org.eclipse.birt.data.oda
	 * .SortSpec)
	 */
